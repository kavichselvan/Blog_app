...
const PostDetails = () => {
  ...
  const fetchPostComments = async () => {
    setLoader(true)
    try {
      const res = await axios.get(`${process.env.REACT_APP_URL}/api/comments/post/${postId}`)
      ...
      {loader ? (
        ...
        post && Object.keys(post).length > 0 ? ( // Add this check
          <div className="px-8 md:px-[200px] mt-8">
            ...
            <img src={`${process.env.REACT_APP_IF}${post.photo}`} className="w-full mx-auto mt-8" alt="" />
          ...
          <div className="h-[80vh] flex justify-center items-center w-full">Post not found</div> // Fallback if post is null or empty
        )
      )}
      <Footer />
    </div>
